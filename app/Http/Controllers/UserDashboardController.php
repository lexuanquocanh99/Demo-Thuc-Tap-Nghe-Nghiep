...
class UserDashboardController extends Controller
{
    ...
    public function profileUpdate(Request $request,$id){
        ...
        if($status){
            request()->session()->flash('success','Hồ sơ đã được cập nhập');
        }
        else{
    ...
    public function userOrderDelete($id)
    {
        ...
        if($order){
            if($order->status=="process" || $order->status=='delivered' || $order->status=='cancel'){
                return redirect()->back()->with('error','Bạn không thể xóa đơn hàng');
            }
            else{
                $status=$order->delete();
                if($status){
                    request()->session()->flash('success','Đơn hàng đã được xoá');
                }
                else{
    ...
    public function productReviewUpdate(Request $request, $id)
    {
        ...
        return redirect()->route('user.product-review.index');
    }
    ...
    public function userCommentDelete($id){
        ...
        if($comment){
            ...
            if($status){
                request()->session()->flash('success','Bình luận đã được xoá');
            }
            else{
                request()->session()->flash('error','Vui lòng thử lại!');
            }
    ...
    public function userCommentUpdate(Request $request, $id)
    {
        ...
        if($comment){
            ...
            if($status){
                request()->session()->flash('success','Bình luận đã được cập nhập');
            }
            else{
                request()->session()->flash('error','Vui lòng thử lại!');
            }
